...
export default function WheelPage(){
  ...
  const labels=useMemo(()=>entries.map((entry,i)=>{
    const slice=360/entries.length;
    const angle=i*slice+slice/2;
    const rad=(angle-90)*Math.PI/180;
    const radius=34;
    return {entry,left:50+Math.cos(rad)*radius,top:50+Math.sin(rad)*radius,angle};
  }),[entries]);
