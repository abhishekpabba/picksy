...
export default function WheelPage(){
  ...
  const slice=entries.length?360/entries.length:360;
  function spin(){if(spinning||entries.length<2)return;const idx=secureIndex(entries.length);const selectedAngle=idx*slice+slice/2;const current=((spinRef.current%360)+360)%360;const desired=(360-selectedAngle)%360;const delta=(desired-current+360)%360;spinRef.current+=360*6+delta;setRotation(spinRef.current);setSpinning(true);setWinner('');setTimeout(()=>{setWinner(entries[idx]);setHistory(h=>[entries[idx],...h].slice(0,12));setSpinning(false)},4200)}
